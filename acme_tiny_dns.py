...
def get_crt(account_key, csr, hook, log=LOGGER, CA=DEFAULT_CA, disable_check=False, directory_url=DEFAULT_DIRECTORY_URL, contact=None):
    ...
    for auth_url in order['authorizations']:
        ...
        log.info("Running {0} {1} {2} {3}".format(hook, 'update', domain, keyauthhash))
        subprocess.check_call([hook, 'update', domain, keyauthhash])

        log.info("DNS record created. Pausing 10 sec so DNS can settle...")
        time.sleep(10)
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        ...
        description=textwrap.dedent("""\
            ...
            Example Usage: python acme_tiny.py --account-key ./account.key --csr ./domain.csr --hook /opt/update_my_zone > signed_chain.crt
            """)
    )
